...
ダガー `†` は、この記事ではほぼ **「逆向きに戻すゲート」** と思って大丈夫です。
...
なぜ `-i` かというと、`i` をかけたあとに `-i` をかけると元に戻るからです。
...
と続けると、何もしなかったのと同じになります。

```text
SS† = I
```

ここで `I` は identity、つまり **何もしないゲート** です。
...
以下は記事にそのまま挿入できる **コラム原稿（Markdown形式）** です。
...
のように、ゲートの右上に付く `†` 記号を **ダガー（dagger）** と呼びます。

この名前の由来は、とても単純です。

見た目が「短剣（dagger）」に似ているからです。

```
†
```

この記号はもともと数学よりも古く、写本や印刷物の世界で
